...
#define LEDA_ROOT_INCL_ID 700328
...
class __exportGeoW  GeoWinTypeName;
...
as points, lines, segments, arrows, circles, polygons, graph, \dots and 
...
There are 18 predefined colors (enumeration in \<LEDA/graphics/x\_window.h\>):
...
\item
The {\em grid dist} parameter (default 0) defines the width of the
...
\item 
The {\em show orientation} parameter defines, whether or not the direction
or orientation of segments, lines, rays, triangles, polygons and gen\_polygons
...
static int dpi() { return BASE_WINDOW::screen_dpi(); }
static float scaling() { return BASE_WINDOW::screen_scaling(); }
...
/*{\Mopl     sets $xmin$ to $x_0$, $xmax$ to $x_1$, and $ymin$ to $y_0$, 
             ...
             $ymax$ to $ymin + h/scaling$. Here $w$ and $h$ are the
             width and height of the drawing section in pixels. }*/

/*{\Moptions nextwarning=no}*/
void init_rectangle(double x0, double x1, double y0, double y1) 
{ BASE_WINDOW::init(x0,x1,y0,y1); }
void init(double x0, double x1, double y0, double y1) 
{ BASE_WINDOW::init(x0,x1,y0,y1); }
/*{\Mopl     adjusts the window such that the points $(x_0,y_0)$ and 
             $(x_1,y_1)$ are contained in the drawing section. }*/
...
void display() { BASE_WINDOW::display(window::center,window::center,0); }
/*{\Mopl      opens $W$ and displays it at the center of the screen.
              Note that $W$.display() has to be called before
...
/*{\Moptions nextwarning=no}*/
void display(window& W0, int x, int y) { BASE_WINDOW::display(x,y,&W0); }
...
void open_hidden() { BASE_WINDOW::display(0, 0, 0, true); }
...
bool set_show_orientation(bool orient)
/*{\Mop      sets the show orientation parameter to $orient$.}*/
{ bool prev = show_orientation; show_orientation = orient;
  return prev; 
}
...
void set_icon_label(string s){ BASE_WINDOW::set_icon_label(s); }
...
//bool load_text_font(string fn)    { return BASE_WINDOW::load_text_font(fn)!=0; }
/*{\Xopl      loads $X11$ font $fn$ as text font. Returns true on success and 
              false if the font is not available.}*/

//bool load_bold_font(string fn)    { return BASE_WINDOW::load_bold_font(fn)!=0; }
/*{\Xopl      loads $X11$ font $fn$ as bold font. Returns true on success and 
              false if the font is not available.}*/

//bool load_fixed_font(string fn) { return BASE_WINDOW::load_fixed_font(fn)!=0;  }
/*{\Xopl      load $X11$ font $fn$ as fixed font. Returns true on success 
              and false if the font is not available.}*/

//bool load_button_font(string fn) { return BASE_WINDOW::load_button_font(fn)!=0;}
...
bool get_show_orientation() const
/*{\Mop      returns the show orientation parameter.}*/
{ return show_orientation; }
...
/*{\Mop   returns a pointer to the |GeoWin| (see Section \ref{GeoWin})
          that uses |\Mvar| as its display window or |NULL| if |\Mvar| 
...
int menu_bar_height() const { return BASE_WINDOW::mb_height(); }
/*{\Mop      returns the height of the menu bar of $W$ in pixels
             and $0$ if $W$ has no menu bar (see |W.make_menu_bar()|). }*/
...
/*{\Mop returns true if $p$ lies in the drawing area. }*/
...
void draw_pixels(const list<point>& L, color c=window::fgcol);
...
void draw_segment(point p, point q, line l, color c = window::fgcol);
/*{\Mop draws the part of the line $l$ between $p$ and $q$. This version of 
        |draw_segment| should be used if $p$ or $q$ may lie far outside $W$. 
        \precond $p$ and $q$ lie on $l$ or at least close to $l$. }*/


void draw_segments(const list<segment>& L, color c=window::fgcol);
...
void draw_ray(point p, point q, line l, color c = window::fgcol);
/*{\Mop draws the part of the line $l$ on the ray with source 
        $p$ and passing through $q$. This version of |draw_ray| 
        should be used if $p$ may lie far outside $W$. \precond $p$ 
        and $q$ lie on $l$ or at least close to $l$. }*/
...
static void compute_bezier(const list<point>&, int,double*,double*);
static void bezier_segments(const list<point>&, int,segment&,segment&);

void draw_arc(point, point, point, int arr, double d, color);
void draw_bezier(const list<point>&, int n, int arr,double d,bool border,color);
void draw_spline(const list<point>&, int n, int arr,double d,bool border,color);


void draw_arc(const point& p, const point& q, const point& r, color c=window::fgcol);
/*{\Mopl     draws a circular arc starting in $p$ passing through $q$ and 
             ending in $r$. }*/

void draw_bezier(const list<point>& C, int n,  color c=window::fgcol);
/*{\Mopl     draws the bezier curve with control polygon $C$ by a polyline 
             with $n$ points. }*/


void draw_spline(const list<point>& L, int n,  color c=window::fgcol);
/*{\Mopl     draws a spline curve through the points of $L$. Each segment
             is approximated by a polyline with $m$ points. }*/

void draw_closed_spline(const list<point>& L, int n,  color c=window::fgcol);
/*{\Mopl     draws a {\em closed} spline through the points of $L$. }*/

void draw_spline(const polygon& P, int n,  color c=window::fgcol);
/*{\Mopl     draws a {\em closed} spline through the vertices of $P$. }*/
...
void draw_filled_arrow(const list<point>& L, int arrow, double d, bool border,
                                                        color=window::fgcol);
...
void draw_polyline_arrow(const list<point>& lp, color c=window::fgcol);
/*{\Mopl     draws a polyline arrow with vertex sequence $lp$.}*/


void draw_arc_arrow(const point& p, const point& q, const point& r, color c=window::fgcol);
/*{\Mopl     draws a circular arc arrow starting in $p$ passing through $q$ 
             and ending in $r$. }*/

void draw_bezier_arrow(const list<point>& C, int n,  color c=window::fgcol);
/*{\Mopl     draws the bezier curve with control polygon $C$ by a polyline 
             with $n$ points, the last segment is drawn as an arrow. }*/


void draw_spline_arrow(const list<point>& L, int n,  color c=window::fgcol);
/*{\Mopl     draws a spline curve through the points of $L$. Each segment
             is approximated by a polyline with $n$ points. The last
             segment is drawn as an arrow. }*/

static point arrow_head(const point&, double, double, double*, double*);
static int   arrow(const point&, const point&, double, int, double*, double*);
static int   arrow(const list<point>&, double, int, double*, double*);

point draw_arrow_head(const point& p, double dir, double d, color c);
...
void draw_filled_circle(double x, double y, double r, color c=window::fgcol)
{ draw_disc(x,y,r,c); }
/*{\Mopl     draws a filled circle with center $(x,y)$ and radius $r$.}*/

void draw_filled_circle(const point& p, double r, color c=window::fgcol)
{ draw_disc(p,r,c); }
/*{\Mopl     draws a filled circle with center $p$ and radius $r$.}*/

void draw_filled_circle(const circle& C, color c=window::fgcol)
{ draw_disc(C,c); }
/*{\Mopl     draws filled circle $C$.}*/
...
void draw_polyline(const list<point>& lp, int arrow, double d, color c);

void draw_polyline(const list<point>& lp, const list<color>& clr);

void draw_polyline(const list<point>& lp, color c=window::fgcol);
...
void draw_oriented_polygon(const list<point>& lp, color c=window::fgcol);
...
void draw_oriented_polygon(const polygon& P, color c=window::fgcol);
/*{\Mopl     draws polygon $P$ and indicates the orientation by an arrow.}*/
...
void clip_mask_polygon(const polygon& pol, int mode);
void clip_mask_polygon(const list<point>& L, int mode);
void clip_mask_polygon(int n, double* xc, double* yc, int mode);


// generalized polygons

void draw_polygon(const gen_polygon& P, color c=window::fgcol);
/*{\Mopl     draws polygon $P$.}*/

void draw_oriented_polygon(const gen_polygon& P, color c=window::fgcol);
/*{\Mopl     draws polygon $P$ and indicates the orientation by an arrow.}*/

void draw_filled_polygon(const gen_polygon& P, color c=window::fgcol);
/*{\Mopl     draws filled polygon $P$.}*/
...
void draw_rectangle(const rectangle& R, color=window::fgcol);
/*{\Mopl     draws rectangle $R$.}*/
...
void draw_filled_rectangle(const rectangle& R, color=window::fgcol);
/*{\Mopl     draws rectangle $R$.}*/
...
void draw_box(const rectangle& R, color c=window::fgcol)
{ draw_filled_rectangle(R,c); }
/*{\Mopl     same as |draw_filled_rectangle(p,q,c)|. }*/
...
/*{\Mopl     draws a rectangle $(x_0,y_0,x_1,y_1)$ with round corners.
             The |rndness| argument must be a real number in the interval
             $[0,1]$ and defines the ``roundness'' of the rectangle. }*/

void draw_roundrect(point p, point q, double rndness, color col=window::fgcol)
{ draw_roundrect(p.xcoord(),p.ycoord(),q.xcoord(),q.ycoord(),rndness,col); }
...
void draw_roundbox(point p, point q, double rndness, color col=window::fgcol)
{ draw_roundbox(p.xcoord(),p.ycoord(),q.xcoord(),q.ycoord(),rndness,col); }
...
void draw_triangle(const triangle& T, color=window::fgcol);
/*{\Mopl     draws triangle $T$. }*/

void draw_filled_triangle(point a, point b, point c, color=window::fgcol);
/*{\Mopl     draws filled triangle $(a,b,c)$. }*/

void draw_filled_triangle(const triangle& T, color=window::fgcol);
/*{\Mopl     draws filled triangle $T$. }*/
...
{ return BASE_WINDOW::create_pixrect_from_color(w,h,clr); }
/*{\Mopl   creates a a solid pixrect of width $w$ und height $h$. }*/


char* create_pixrect_from_xpm(const char** xpm_str)
{ return BASE_WINDOW::create_pixrect_from_xpm(xpm_str); }
/*{\Mopl   creates a pixrect from the {\bf xpm} data string |xpm_str|. }*/


char* create_pixrect(const char** xpm_str) {
   return create_pixrect_from_xpm(xpm_str);
}
/*{\Mopl   creates a pixrect from the {\bf xpm} data string |xpm_str|. }*/


char* create_pixrect_from_xpm(string xpm_file);
/*{\Mopl   creates a pixrect from the {\bf xpm} file |xpm_file|. }*/


char* create_pixrect(string xpm_file) {
   return create_pixrect_from_xpm(xpm_file);
}
/*{\Mopl   creates a pixrect from the {\bf xpm} file |xpm_file|. }*/


char* create_pixrect_from_bits(int w, int h, unsigned char* bm_data, int fg=window::fgcol,
                                                           int bg=window::bgcol)
{ return BASE_WINDOW::create_pixrect_from_bits(w,h,bm_data,fg,bg); }
...
int get_pixrect_width(char* pr) const { return BASE_WINDOW::get_pixrect_width(pr); }
/*{\Mopl   returns the width (number of pixels in a row) 
           of pixrect |pr|. }*/

int get_pixrect_height(char* pr) const { return BASE_WINDOW::get_pixrect_height(pr); }
...
void  put_pixrect(point p, char* pr)
{ BASE_WINDOW::put_pixrect(p.xcoord(),p.ycoord(),pr); }
/*{\Mopl   copies the contents of pixrect $pr$ with lower left corner at
           position $p$ into $W$. }*/
...
void  center_pixrect(char* pr)
{ BASE_WINDOW::center_pixrect(pr); }
/*{\Mopl   copies the contents of pixrect $pr$ into $W$ such that its
           center lies on the center of $W$. }*/



void  put_pixrect(char* pr) { BASE_WINDOW::put_pixrect(xmin(),ymin(),pr); }
/*{\Mopl   copies pixrect $pr$ with lower left corner at
           position |(W.xmin(),W.ymin())| into $W$. }*/

void  set_pixrect(char* pr) { BASE_WINDOW::set_pixrect(pr); }
/*{\Mopl   copies pixrect $pr$ with upper left corner at
           position $(0,0)$ into $W$. }*/

//void  fit_pixrect(char* pr) { BASE_WINDOW::set_pixrect(pr); }
//
void  fit_pixrect(char* pr) { BASE_WINDOW::fit_pixrect(pr); }
/*{\Mopl   scales pixrect $pr$ to fit into $W$. }*/
...
void set_clip_polygon(const list<point>&);
void set_clip_polygon(const polygon& P) { set_clip_polygon(P.vertices()); }
*/
...
the event type (enumeration in \<LEDA/graphics/x\_window.h\>), the value of 
...
void await_button(int b = MOUSE_BUTTON(1)) { while (read_mouse() != b); }
void await_mouse_button(int b = 1) { await_button( MOUSE_BUTTON(b) ); }
...
int read_mouse_line(const point& p, point& q);
/*{\Mopl     as above with |x0 = p.xcoord()| and |y0 = p.ycoord()|
             and the current position is assigned to |q|. }*/
...
int read_mouse_ray(const point& p, point& q);
/*{\Mopl     as above with |x0 = p.xcoord()| and |y0 = p.ycoord()|
             and the current position is assigned to |q|. }*/
...
int read_mouse_arc(double x0, double y0, double x1, double y1, double& x, double& y);
/*{\Mopl     displays an arc that starts in $(x_0,y_0)$, ends in
             $(x_1,y_1)$ and passes through the current cursor 
			 position. When a mouse button is pressed inside the 
			 drawing section of $W$, the current position is 
			 assigned to $(x,y)$ and the number of the pressed 
			 button is returned.}*/

int read_mouse_arc(const point& p, const point& q, point& r);
/*{\Mopl     as above with $(x0,y0) = p$ and $(x1,y1) = q$
             and the current position is assigned to |r|. }*/

int read_mouse_arc_with_fixed_src(const point& src, point& tgt, point& middle);
...
int read_mouse(point& p, int timeout1, int timeout2, 
                         bool& double_click, bool& drag)
{ double x = p.xcoord();
  double y = p.ycoord();
  int k = read_mouse(x,y,timeout1,timeout2,double_click,drag);
  p = point(x,y);
  return k;
}
/*{\Mop  ... }*/
...
/*{\Mopl   waits for next event in window $W$ and returns it. 
           ...
           Possible events are (cf. \<LEDA/graphics/x\_window.h\>): 
...
window& draw(const rectangle& R, color c=window::fgcol);
window& draw(const polygon& P,color c=window::fgcol);
window& draw(const gen_polygon& P,color c=window::fgcol);
window& draw(const triangle& T,color c=window::fgcol);
...
window& operator<<(const gen_polygon& P) { return draw(P); }
/*{\Mbinop   like $W$.draw\_polygon($P$).}*/

window& operator<<(const rectangle& R) { return draw(R); }
/*{\Mbinop   like $W$.draw\_rectangle($R$).}*/

window& operator<<(const triangle& T) { return draw(T); }
/*{\Mbinop   like $W$.draw\_triangle($T$).}*/
...
window& read(circle&);
window& read(polygon&);
window& read(gen_polygon&);
window& read(rectangle&);
window& read(triangle&);

window& operator>>(point& p);
/*{\Mbinop   reads a point $p$: clicking the left button 
	     assigns the current cursor position to $p$.}*/

void set_point_buffer(const point p);

window& operator>>(segment& s);
/*{\Mbinop   reads a segment $s$: use the left button to define 
	     the start and end point of $s$.}*/

window& operator>>(ray& r);
/*{\Mbinop   reads a ray $r$: use the left button to define 
	     the start point and a second point on $r$.}*/

window& operator>>(line& l);
/*{\Mbinop   reads a line $l$: use the left button to define 
	     two different points on $l$.}*/

window& operator>>(circle& C);
/*{\Mbinop   reads a circle $C$: use the left button to define 
	     the center of $C$ and a point on $C$.}*/

window& operator>>(rectangle& R);
/*{\Mbinop   reads a rectangle $R$: use the left button to define 
	     two opposite corners of $R$.}*/
	     
window& operator>>(triangle& T);
/*{\Mbinop   reads a triangle $T$: use the left button to define 
	     the corners of $T$.}*/


window& operator>>(polygon& P);
/*{\Mbinop   reads a polygon $P$: use the left button to define 
	     the sequence of vertices of $P$, end the sequence 
	     by clicking the right button.}*/

window& operator>>(gen_polygon& P);
/*{\Mbinop   reads a generalized polygon $P$; input the polygons
             defining $P$ and end the input by clicking the middle
	     button.}*/


list<point> read_polygon();
/*{\Mopl    as above, however, returns list of vertices. }*/

window& read_arc(point& src, point& tgt, point& middle);
...
{ return BASE_WINDOW::read_event((BASE_WINDOW*&)w,val,x,y,t,timeout); }
...
/*\{\Mfuncl waits for next event and returns it. Assigns the window to
            ...
            $(x,y)$. Possible events are (cf. <LEDA/graphics/x_window.h>): 
...
void set_item_width(panel_item it, int w) { BASE_WINDOW::set_item_width(it,w); }
void set_item_height(panel_item it,int h){ BASE_WINDOW::set_item_height(it,h); }
...
/*
panel_item lwidth42_item(string s, int& x, const char* hlp=0);
panel_item lwidth42_item(string s, int& x, void(*F)(int), const char* hlp=0);
panel_item lwidth42_item(string s, int& x, const window_handler& obj, const char* hlp=0);
*/
...
panel_item string_item(string s, string& x, const list<string>& L, int sz,const char* hlp=0);
/*{\Mopl  menu |L| is displayed in a scroll box of height |sz|. }*/


panel_item string_item(string s, string& x, const list<string>& L, int sz,
                                            void (*F)(char*),const char* hlp=0);
/*{\Mopl     as above with action function |F|.}*/
...
void set_menu(panel_item it, const list<string>& L, int sz=0);
...
// synonym
void add_menu(panel_item it, const list<string>& L, int sz=0) 
{ set_menu(it,L,sz); }
...
panel_item choice_mult_item(string s,int& x,string s1, const char* hlp=0);
/*{\Mopl  }*/

panel_item choice_mult_item(string s,int& x,string s1, string s2, const char* hlp=0);
/*{\Mopl  }*/
...
panel_item choice_mult_item(string s,int& x,const list<string>& L,const window_handler& obj, 
                                                            const char* hlp=0);
/*{\Mopl  }*/
...
void disable_buttons() { BASE_WINDOW::disable_buttons(); }
/*{\Mopl  disables all buttons. }*/

void enable_buttons() { BASE_WINDOW::enable_buttons(); }
/*{\Mopl  enables all buttons. }*/
...
panel_item  get_item(string s) { return BASE_WINDOW::get_item(s); }
/*{\Mopl  returns the item with label $s$ and |NULL| if no such
          item exists in $W$. }*/


int  get_button(string s) { return BASE_WINDOW::get_button(s); }
...
{ BASE_WINDOW::set_button_label(but,s); }
...
window* set_window(panel_item it, window* M);
...
/*{\Mtext
\bigskip
{\bf 3.9.9. Miscellanous}
}*/

void redraw_panel() { BASE_WINDOW::redraw_panel(); }
/*{\Mopl     redraw the panel area of $W$. }*/

void redraw_panel(panel_item it) { BASE_WINDOW::draw_panel_items(it); }
...
void set_tooltip(int i, double x0, double y0, double x1, double y1, string txt)
{ BASE_WINDOW::set_tooltip(i,x0,y0,x1,y1,txt); }
/*{\Mopl inserts a tooltip with id $i$, rectangle ($x_0$, $y_0$,$x_1$,$y_1$)
         and text $txt$ into the window. The text is shown when the mouse 
         pointer enters the rectangle. The text disappears as soon as the 
         mouse pointer leaves the rectangle.\\
         CAUTION: Currently the method has to be called after the call of
         $W$.display(). Setting a tooltip before the call $W$.display() has no
         effect.}*/

void del_tooltip(int i) { BASE_WINDOW::del_tooltip(i); }
/*{\Mopl     removes the tooltip with id $i$.}*/


void string_edit(point p, string& s);
...
bool read_zoom_rect(const point& center,point& p, point& q );
bool read_zoom_rect(point& p, point& q);
...
void zoom_area(const point& p, const point& q, int steps = 16, 
                                     void (*redraw)(window*) = 0)
{ zoom_area(p.xcoord(),p.ycoord(),q.xcoord(),q.ycoord(),steps,redraw); }

void zoom(double f, int steps = 16, void(*redraw)(window*) = 0);

void scroll_window(const point& p, 
...
extern __exportF void LedaFileViewer(window& W, string fname, string dname, string pat);
extern __exportF void LedaFileViewer(window& W, string fname, string dname);
extern __exportF void LedaFileViewer(window& W, string fname);


extern __exportF void LedaFileViewer(string fname, string dname, string pat);
extern __exportF void LedaFileViewer(string fname, string dname);
extern __exportF void LedaFileViewer(string fname);


extern __exportF void LedaFileViewer(const list<string>& L, int w, int h, 
                                                               string label);

extern __exportF void LedaFileViewer(window& W, const list<string>& L);

extern __exportF void LedaFileViewer(const list<string>& L);
...
#if (defined(__win32__) || defined(__win64__)) && defined(WINMAIN)
