...
#define LEDA_ROOT_INCL_ID 700321
...
\normalsize 
  ...
  The parameters of |ev_fcn| are the window causing the event, the event that occurred, 
...
public:
  ...
  void results_handle(const GeoObj& t)
  {
     ...
     else this->draw_object_with_properties(t); 
   } 
  ...
  void scene_read_object(int srk)
  {
    ...
    if (srk == 0){
     ...
     if (post_window_default_input_handler) post_window_default_input_handler((*gw),t);
      ...
      but = this->keyboard_panel(gw->get_window(), nval);
	...
	forall(obj, clip_objs)
	  {
	    ...
	    this->draw_object_with_properties(obnew);
	  }
...
#define update base_class::update
  ...
  { 
	...
	if (b) { // move
	 if ( !pre_move_fcn  || (pre_move_fcn(*gw,obj1,dx,dy))){
	  changed.append(obj1);
	  if (!obj_defining_points_change) this->move( obj1, dx, dy);
  ...
  virtual void obj_focus(){
	...
	GeoObj& obj = *mouse_obj; 
        ...
        but= this->focus_dialog(nvl, gw->get_window());
