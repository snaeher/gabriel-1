...
#define LEDA_ROOT_INCL_ID 700270
...
#if defined(_WIN64) || defined(WORD_LENGTH_64)
...
An undirected graph may not contain self-loops, i.e., it may not contain an 
...
$adj\_edges(v)$ (removing self-loops).  Conversely, $G$.make\_directed() 
...
/*{\Mopl      the nodes of $G$ are sorted according to the entries of
              ...
              \precond $T$ must be numerical, i.e., number type |int, float, double, integer, rational or real|. }*/
...
LEDA_FORALL_PREAMBLE \
for(leda::edge loop_var=First_Adj_Edge(v,0);e=loop_var,\
...
LEDA_FORALL_PREAMBLE \
for(leda::edge loop_var=First_Adj_Edge(v,0);\
...
for(e=leda::graph_of(F)->first_face_edge(F); e; e=leda::graph_of(F)->next_face_edge(e,F))

#define forall_adj_faces(f,v)\
LEDA_FORALL_PREAMBLE \
for(leda::edge loop_var=leda::graph_of(v)->first_adj_edge(v);\
loop_var && (f = leda::graph_of(v)->adj_face(loop_var)) != 0;\
loop_var=leda::graph_of(v)->adj_succ(loop_var))
...
compute\_faces, all\_faces, make\_bidirected, clear, write, and read which
