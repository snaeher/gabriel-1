...
#define LEDA_ROOT_INCL_ID 661317
...
leda::error_handler(i,string("%s (%s:%d)",msg,__FILE__,__LINE__))
*/
#define LEDA_EXCEPTION(i,msg) \
leda::error_handler(i,msg + string(" (%s:%d)",__FILE__,__LINE__))
...
if (!(cond)) LEDA_EXCEPTION(1, string("LEDA PRECONDITION VIOLATION: ")+fstr); 
#else
#define LEDA_OPT_PRECOND(cond,fstr)
#endif
