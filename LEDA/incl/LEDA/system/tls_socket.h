...
int  receive(char* buf, int sz, int sec);
bool poll(int msec);
...
void tls_socket::send(char* buffer, int size) { 
...
int  tls_socket::receive(char* buf, int sz) { 
  return SOCK->receive(buf,sz); 
}

int  tls_socket::receive(char* buf, int sz, int sec) { 
 if (poll(1000*sec)) 
