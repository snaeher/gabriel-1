...
#define LEDA_ROOT_INCL_ID 661293
...
class geowin_import_leda_objects : public geowin_import {
  virtual void operator()(geo_scene sc, string filename) { 
     ...
     if (! is) { LEDA_EXCEPTION(0,"No such file or directory."); return; }
...
void geowin_read_in(ifstream& is, circle& obj, int repres)
{
  ...
  if (repres==0) { // Cartesian
    ...
    double rad = std::sqrt(sr);
    
    obj = circle(xm,ym, rad);
...
{
#if defined(__SUNPRO_CC) || defined(__KCC) || defined(_MSC_VER) || (defined(__GNUC__) && (__GNUC__ >= 3))
 char ch;
#else 
 unsigned char ch;
#endif
...
public:
 ...
 void operator()(GeoWin& gw, list<T>& LC)
 {
  ...
  typename T::point_type p1, p2, p3;
...
public:
 ...
 void operator()(GeoWin& gw, list<T>& LC)
 {
  ...
  typename T::point_type p1(mp), p2(seg.end());
