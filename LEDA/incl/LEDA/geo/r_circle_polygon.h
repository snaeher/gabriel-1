...
#define LEDA_ROOT_INCL_ID 661187
...
r_circle_polygon(const rat_circle& circ, 
                 RESPECT_TYPE respect_orient = RESPECT_ORIENTATION);
/*{\Mcreate creates a polygon |\Mvar| whose boundary is the circle |circ|.}*/
...
/*{\Moperations}*/
        ...
        bool is_closed_chain() const
        { return is_closed_chain(segments()); }
        /*{\Mop   tests whether |\Mvar| is a closed chain.}*/
        ...
        list<r_circle_polygon> split_into_weakly_simple_parts() const;
        /*{\Mop splits |\Mvar| into a set of weakly simple polygons whose union 
                coincides with the inner points of |\Mvar|.
        ...
        r_circle_polygon round(int prec = 0) const;
        ...
        rat_polygon approximate_by_rat_polygon(double dist) const;
        /*{\Mopl    approximates |\Mvar| by a |rat_polygon|. The maxmum distance
        ...
        static CHECK_TYPE input_check_type;

        static void write(ostream& out, const r_circle_polygon& p);
        static void read(istream& in, r_circle_polygon& p, 
                         CHECK_TYPE check = WEAKLY_SIMPLE, 
                         RESPECT_TYPE respect_orient = RESPECT_ORIENTATION);
...
r_circle_gen_polygon buffer(double d) const;
/*{\Mfunc adds an exterior buffer zone to |\Mvar| (if $d > 0$), 
          or removes an interior buffer zone from |\Mvar| (if $d < 0$). 
          More precisely, for $d \geq 0$ define
          the buffer tube $T$ as the set of all points in the complement of 
          |\Mvar| whose distance to |\Mvar| is at most $d$. Then the function 
          returns |\Mvar| $\cup T$.
          For $d < 0$ let $T$ denote the set of all points in |\Mvar| whose
          distance to the complement is less than $\Labs{d}$. Then the result
          is |\Mvar| $\setminus T$. {\em Note} that the result is a 
          generalized polygon since the buffer of a connected polygon may 
          be disconnected, i.e. consist of several parts, if $d < 0$.
}*/


public:
        static bool is_closed_chain(const list<r_circle_segment>& segs);

        static bool clean_chain(list<r_circle_segment>& segs);

        static CHECK_TYPE check_chain_for_simplicity(const list<r_circle_segment>& segs, 
                                                         list<r_circle_point>* crossings = 0);

        static CHECK_TYPE check_chains_for_simplicity(const list<r_circle_polygon>& polys, 
                                                          list<r_circle_point>* crossings = 0);
        ...
        static int orientation_at_lex_smallest_bend(const r_circle_segment& s_pred, 
...
for(list_item vert_loop_var = leda::LedaForallVerticesFirstItem(P);\
leda::LedaForallVerticesAssign((P),vert_loop_var, v);\
leda::LedaForallVerticesNextItem((P),vert_loop_var))
#endif
