...
#define LEDA_ROOT_INCL_ID 661173
...
extern __exportF list<point> UPPER_CONVEX_HULL1(const list<point>& L);
...
/*{\Mtext
\bigskip
$\bullet$ {\bf Point Location}
\setopdims{1cm}{1.9cm}
}*/


extern __exportF edge LOCATE_IN_TRIANGULATION(const GRAPH<point,int>& G, 
                                              point p, edge start = 0);
/*{\Mfuncl  returns an edge |e| of triangulation |G| that contains |p| or 
   that borders the face that contains $p$. In the former case, 
   a hull edge is returned if $p$ lies on the boundary of the convex hull.
   In the latter case we have 
   |orientation(e,p) > 0| except if all points of |G| are collinear and 
   |p| lies on the induced line. In this case |target(e)| is visible from 
   |p|. The function returns |nil| if |G| has no edge.
   The optional third argument is an edge of |G|, where the |locate| 
   operation starts searching. }*/



extern __exportF edge LOCATE_IN_TRIANGULATION(const GRAPH<point,segment>& G, 
                                              point p, edge start = 0);
/*{\Mfuncl  as above, for constraint triangulations. }*/


extern __exportF edge LOCATE_IN_TRIANGULATION(const graph& G, 
                                              const node_array<point>& pos, 
                                              point p, edge start = 0);
/*{\Mfuncl  as above, for arbitrary graph types representing a triangulation.
    Node positions have to be supplied in a node\_array |pos|. }*/



extern __exportF bool CHECK_LOCATE_IN_TRIANGULATION(const graph& G, 
                                              const node_array<point>& pos, 
                                              point p, edge e);
...
extern __exportF bool Is_Voronoi_Diagram(const GRAPH<circle,point>& G, 
                        delaunay_voronoi_kind kind);
...
/*{\Mfuncl
The nesting tree $T$ of a generalized polygon $P$ is defined as follows.
Every node $v$ in $T$ is labelled with a polygon $T[v]$ from the boundary
representation of $P$, except for root $r$ of $T$ which is labelled with the
empty polygon. The root symbolizes the whole two-dimensional plane.
There is an edge $(u,v)$ (with $u \neq r$) in $T$ iff the bounded region of 
$T[v]$ is directly nested in $T[u]$. The term ''directly´´ means that there 
is no node $w$ different from $u$ and $v$ such that $T[v]$ is nested in 
$T[w]$ and $T[w]$ is nested in $T[u]$. And there is an edge $(r,v)$ iff $T[v]$
is not nested in any other polygon of $P$.
The function computes the nesting tree of $P$ and returns its root. 
(The running time of the function depends on the order of the polygons in the
boundary representation of $P$. The closer directly nested polygons are, the 
better.) }*/
