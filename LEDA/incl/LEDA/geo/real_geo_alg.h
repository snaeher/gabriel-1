...
#define LEDA_ROOT_INCL_ID 661220
...
extern __exportF edge LOCATE_IN_TRIANGULATION(const GRAPH<real_point,int>& G, 
                                              real_point p, edge start = 0);

extern __exportF edge LOCATE_IN_TRIANGULATION(const GRAPH<real_point,real_segment>& G, 
                                         real_point p, edge start = 0);


extern __exportF edge LOCATE_IN_TRIANGULATION(const graph& G, 
                                              const node_array<real_point>& pos,
                                              real_point p, edge start = 0);
...
extern __exportF bool Is_Voronoi_Diagram(const GRAPH<real_circle,real_point>& G, 
                        delaunay_voronoi_kind kind);
...
extern __exportF node Nesting_Tree(const real_gen_polygon& P, GRAPH<real_polygon, int>& T);
