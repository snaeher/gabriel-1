...
#define LEDA_ROOT_INCL_ID 661203
...
public:
	...
	{
		switch (p.kind()) {
		case r_circle_polygon::EMPTY: Kind = EMPTY; break;
		case r_circle_polygon::FULL:  Kind = FULL;  break;
...
/*{\Mopl adds an exterior buffer zone to |\Mvar|($d > 0$),
         or removes an interior buffer zone from |\Mvar| ($d < 0$).
         More precisely, for $d \geq 0$ define the buffer tube $T$ as
         the set of all points in the complement of |\Mvar| whose distance 
         to |\Mvar| is at most $d$. Then the function returns |\Mvar| $\cup T$.
