...
#define LEDA_ROOT_INCL_ID 661071
...
int index_of(string s1, int i) const;
...
int index_of(const string& s1) const { return index_of(s1,0); }
/*{\Mop       returns index_of$(s1,0)$.}*/

int index_of(char c, int i) const { return index_of(string(c),i); }
int index_of(char c) const { return index_of(string(c)); }


int split(string* A, int sz, char sep) const; 
/*{\Mop       splits $s$ into substrings separated by $sep$ characters
              and stores them in the array $A[0..sz-1]$. The operation 
              returns the number of the created substrings (at most $sz$).\\
	      \precond{$A$ is an array of length $sz$.} }*/

int break_into_words(string* A, int sz) const { return split(A,sz,' '); }
/*{\Mop       breaks $s$ into words and stores them in the array $A$.\\
              Same as $s$.split(A,sz,' ')$ }*/

// for backward compatibility
int pos(string s1, int i) const { return index_of(s1,i); }
int pos(string s1) const { return index_of(s1); }
int pos(char c, int i) const { return index_of(c,i); }
int pos(char c) const { return index_of(c); }



/*{\Moptions nextwarning=no}*/
bool contains(const string& s1) const { return index_of(s1) >= 0; }
/*{\Mop       true iff $s1$ is a substring of |\Mvar|.}*/


bool starts_with(const string& s1) const  
{ // return pos(s1,0) == 0; 
  return s1 == substring(0,s1.length()); 
}
/*{\Mop       true iff |\Mvar| starts with $s1$. }*/

bool begins_with(const string& s1) const { return starts_with(s1); }
/*{\Mop       true iff |\Mvar| starts with $s1$. }*/


bool ends_with(const string& s1) const 
{ return s1 == substring(length()-s1.length()); }
/*{\Mop       true iff |\Mvar| starts with $s1$. }*/
...
/*\Mop        returns a upper-character copy of |\Mvar|. }*/ 
