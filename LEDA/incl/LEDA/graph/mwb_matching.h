...
list<edge> MWMCB_MATCHING_T(graph& G, 
                                  ...
                                  const edge_array<NT>& c, 
                                        node_array<NT>& pot);
/*{\Mfunc Returns a maximum weight matching among the matchings of 
maximum cardinality. 
...
bool MWA_SCALE_WEIGHTS(const graph& G, 
                       edge_array<double>& c);
/*{\Mfunc replaces $c[e]$ by |c1[e]| for every edge $e$, where |c1[e]|
was defined above and $f = 4n$. This scaling function should be used for the algorithms that compute minimum of maximum weight assignments or maximum weighted
