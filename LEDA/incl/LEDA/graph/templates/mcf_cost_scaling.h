...
#define LEDA_ROOT_INCL_ID 661135
...
#define PUSH(x,e,w) { push_flow(e,x); G.restore_in_edge(e,w); }
#else
#define PUSH(x,e,w) push_flow(e,x);
#endif
...
void error_handler(int,string msg) { 
    LEDA_EXCEPTION(1,string("mcf_cost_scaling: ") + msg);
 }
...
*/
  ...
  //if (d <=  8)  af_factor = 0.70 * std::sqrt(n);

    if (d <=  8)       af_factor = (9-d) * std::sqrt(n);
    else if (d <= 64)  af_factor = 0.50 * std::sqrt(n);
    else if (d <= 128) af_factor = 0.25 * std::sqrt(n);
    else               af_factor = 0.20 * std::sqrt(n);
   }
...
double eps_factor() const { return eps_f; }
