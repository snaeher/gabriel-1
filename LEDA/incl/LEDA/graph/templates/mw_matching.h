...
#define LEDA_ROOT_INCL_ID 661128
...
{ LEDA_EXCEPTION(1, "Sorry, `max_val' not specified for that type."); }
...
void check_feasibility(const graph &G, 
  ...
  if (!perfect) {
    forall_nodes(u, G) assert(pot[u] >= 0);
  }
...
conc_pq_tree<a,b>::conc_pq_tree() {  
  if (a >= 2 && b >= 2*a-1) {
    ...
    LEDA_EXCEPTION(1, "conc_pq_tree::conc_pq_tree(): invalid arguments for a and b.");
}
...
conc_pq_tree<a,b>::conc_pq_tree(GenPtr p, GenPtr i) {  
  if (a >= 2 && b >= 2*a-1) {
    ...
    LEDA_EXCEPTION(1, "conc_pq_tree::conc_pq_tree(GenPtr p, GenPtr i): invalid arguments for a and b.");
}
...
expand_t = augment_t = scan_t = destroy_t = 0;
  ...
  switch(heur) {
    ...
    case 1: { free = greedy_matching(G, w, pot, mate, perfect); 
              break; 
             }
    case 2: { free = jump_start(G, w, pot, mate, perfect); 
              break; 
             }      
   default: { LEDA_EXCEPTION(1, 
                 "MAX_WEIGHT_MATCHING: illegal heuristic parameter."); 
               break;
             }
  }
  #ifdef _STAT
...
public:
  ...
  blossom(node ba = nil) : concat_pq<NT, vertex<NT>*>() {
    ...
    base_class::set_owner(leda_cast(this));
    label = even;
    pot = offset = 0; 
    base = ba; mate = nil;
    disc = pred = nil;
  ...
  NT min_prio() const 
  { return (base_class::find_min() ? base_class::prio(base_class::find_min()) : MAX_VALUE(NT)); }
  ...
  void expand(NT Delta) {
    ...
    blossom<NT> *CUR=0;
    forall(CUR, subblossom_p()) {
      base_class::split_at_item(CUR->split_item(), *CUR, *this);
      CUR->offset = offset;
      CUR->label = label;
  ...
  bool delete_connection(c_pq_item it, node r) {
    ...
    { if (base_class::inf(it)->empty()) 
        ...
        base_class::increase_p(it, base_class::inf(it)->min_prio());
     }
    return old_min != min_prio();
  }
  ...
  bool improve_connection(c_pq_item it, NT x, node u, node r) {
    ...
    if (!it) return false;
    NT old_min = min_prio();
    if (base_class::inf(it)->decrease_p1(u, x, r)) base_class::decrease_p(it, x);    
    return old_min != min_prio();    
  }
...
public:
  ...
  node min_inf() const 
  { return (base_class::find_min() ? base_class::inf(base_class::find_min()) : nil); }

  bool decrease_p1(node u, NT x, node r) {
    
    NT old_min = min_prio();
    if (!ITEM_OF.defined(r)) 
      ITEM_OF[r] = base_class::insert(x, u);
    else {
      pq_item it = ITEM_OF[r];
      if (base_class::prio(it) > x) {
  ...
  void adjust_priorities(NT adjustment) {
    ...
    forall_defined(r, ITEM_OF) {
      it = ITEM_OF[r];
      cur_prio = base_class::prio(it);
      if (adjustment < 0)
        p_queue<NT, node>::decrease_p(it, cur_prio + adjustment);
      else {  // simulate increase_p
        node v = base_class::inf(it);
        base_class::del_item(it);
        ITEM_OF[r] = base_class::insert(cur_prio + adjustment, v);
      }
    }
  }
...
expand_t = augment_t = scan_t = destroy_t = 0;
  ...
  edge        e;
  node        resp, opst, cur,  adj, u, v      /*, r */;
  blossom<NT> *RESP=0, *OPST=0, *CUR=0, *ADJ=0 /*, *U, *V, *R */;
  ...
  forall_nodes(v, G) 
  { blossom_of(item_of[v],NT(0))->destroy();
    //_BLOSSOM_OF(v)->destroy();
   }
