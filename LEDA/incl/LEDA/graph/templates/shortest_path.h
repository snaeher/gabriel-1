...
#define LEDA_ROOT_INCL_ID 661127
...
// new functions for k-shortest path

template <class T>
T maxval(const graph& G, const node_array<T>& na)
{
	node v;
	T m(0);
	forall_nodes(v, G)
...
template <class T>
T maxval(const graph& G, const edge_array<T>& na)
{
	edge e;
	T m(0);
	forall_edges(e, G)
...
template<class T>
T path_length(const edge_array<T>& c, const list<edge>& path)
{
	T sum(0);
	edge e;
...
// NSPR(int), ANSPR0, for k-path
template <class T>
bool NEAR_SHORTEST_PATHS_RESTRICTED(graph& G, node s, node t, const edge_array<T>& c, const double& eps, list<list<edge>* >& shortest_paths, int k)
{
	G.rev_all_edges();
	node_array<T> dist(G);
	node_array<edge> pred(G);
	node_array<edge> next_edge(G);
	node_array<T> L(G);
	node_array<int> tau(G,0);
	tau[s] = 1;
	double d;
	node v;
	node u;
	...
	if (!SHORTEST_PATH_T(G, t, c, dist, pred))
		return false;
	...
	forall_nodes(v, G)
	{
			next_edge[v]= G.first_adj_edge(v);
	}
	...
	while(!node_stack.empty())
	{
		...
		if(next_edge[u] != nil)
		{
			e = next_edge[u];
			v = G.target(e);
			next_edge[u] = G.adj_succ(e);
			
			if(tau[v] == 0 && (L[u] + c[e] + dist[v] <= d))
			{
				if (v == t)
				{
					node_stack.push(v);
					edge_stack.push(e);
//					print_path(edge_stack, G);
/*
					report_path(shortest_paths, edge_stack);
*/
					k--;
					node_stack.pop();
					edge_stack.pop();
// we could store the following information in a field
//					cout << "Length: " << L[u] + c[e] << endl;
					if (k<0)
						return true;
				}
				else
		...
		else
		{
			...
			if (s != u)
				edge_stack.pop();
			next_edge[u] = G.first_adj_edge(u);
			tau[u]--;
			// here is a dummy step 3
		}
	}
	return true;
}
...
// NSPR(int), ANSPR0
template <class T>
bool NEAR_SHORTEST_PATHS_RESTRICTED(graph& G, node s, node t, const edge_array<T>& c, const double& eps, list<list<edge>* >& shortest_paths)
{
	G.rev_all_edges();
	node_array<T> dist(G);
	node_array<edge> pred(G);
...
// NSPU(int)
template <class T>
bool NEAR_SHORTEST_PATHS_UNRESTRICTED(graph& G, node s, node t, int m, const edge_array<T>& c, const double& eps, list<list<edge>* >& shortest_paths)
{
	G.rev_all_edges();
	node_array<T> dist(G);
	node_array<edge> pred(G);
	node_array<array<edge> > next_edge(G);
	node_array<T> L(G);
	node_array<int> tau(G,0);
	tau[s] = 1;
	int i;
	...
	forall_nodes(v, G)
	{
		(next_edge[v]).resize(1, m);
		for(i = 1; i <= m; i++)
			(next_edge[v])[i] = G.first_adj_edge(v);
	}
	...
	{
		u = node_stack.top();
		if((next_edge[u])[tau[u]] != nil)
		{
			e = (next_edge[u])[tau[u]];
			v = G.target(e);
			(next_edge[u])[tau[u]] = G.adj_succ(e);
			if(L[u] + c[e] + dist[v] <= d)
			{
				// here is a dummy step 1
				if (v == t)
				{
					node_stack.push(v);
					edge_stack.push(e);
...
//					cout << "Length: " << L[u] + c[e] << endl;
		...
		else
		{
			...
			if (s != u)
				edge_stack.pop();
			(next_edge[u])[tau[u]] = G.first_adj_edge(u);
			tau[u]--;
			// here is a dummy step 3
		}
	}
