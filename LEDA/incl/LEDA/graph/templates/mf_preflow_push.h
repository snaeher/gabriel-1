...
#define LEDA_ROOT_INCL_ID 661133
...
string name() { return "Maxflow Preflow-Push"; }

NT    flow()   { return fval; }
NT    result() { return fval; }
...
{ 
  ...
  excess[t] += 1;  // prevents t from beeing inserted into U
...
{ max_flow<NT,graph> mf;
  ...
  if (!ok) LEDA_EXCEPTION(1,string("CHECK_MAX_FLOW_T: ") + msg + ".");
  return ok;
}
