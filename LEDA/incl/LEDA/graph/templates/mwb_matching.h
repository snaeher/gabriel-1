...
#define LEDA_ROOT_INCL_ID 661129
          ...
          const edge_array<NT>& c,node_array<NT>& pot)
{
  ...
  if ( !Is_Bipartite(G,A,B) )
    LEDA_EXCEPTION(1,"MAX_WEIGHT_BIPARTITE_MATCHING: \
G is not bipartite");
          ...
          const edge_array<NT>& c,node_array<NT>& pot)
{
  ...
  if ( !Is_Bipartite(G,A,B) )
    LEDA_EXCEPTION(1,"MAX_WEIGHT_ASSIGNMENT: \
G is not bipartite");
          ...
          const edge_array<NT>& c,node_array<NT>& pot)
{
  ...
  if ( !Is_Bipartite(G,A,B) )
    LEDA_EXCEPTION(1,"MIN_WEIGHT_ASSIGNMENT: \
G is not bipartite");
