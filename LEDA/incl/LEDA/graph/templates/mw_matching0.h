...
#define LEDA_ROOT_INCL_ID 661139
...
#include <LEDA/core/map.h> // only for debug
...
{ LEDA_EXCEPTION(1, "Sorry, `get_max_val' not specified for that type."); }
...
public:
  ...
  void conc(conc_pq_tree &pq, int dir = leda::behind);  
  ...
  conc_pq_node* stl_pred_item(conc_pq_node *p) const {  
    LEDA_EXCEPTION(1,"conc_pq_tree::stl_pred_item not implemented."); 
    return nil; 
  }   
...
conc_pq_tree::conc_pq_tree(int _a, int _b) {  
  if (_a >= 2 && _b >= 2*_a-1) {
    ...
    LEDA_EXCEPTION(1, "conc_pq_tree::conc_pq_tree(int _a, int _b): invalid arguments for _a and _b.");
}
...
conc_pq_tree::conc_pq_tree(GenPtr p, GenPtr i, int _a, int _b) {  
  if (_a >= 2 && _b >= 2*_a-1) {
    ...
    LEDA_EXCEPTION(1, "conc_pq_tree::conc_pq_tree(GenPtr p, GenPtr i, int _a, int _b): invalid arguments for _a and _b.");
}
...
conc_pq_tree::conc_pq_tree(const conc_pq_tree& pq) {
  LEDA_EXCEPTION(1, "conc_pq_tree::conc_pq_tree(const conc_pq_tree& pq): not implemented yet.");
}


conc_pq_tree& conc_pq_tree::operator=(const conc_pq_tree& pq) {
  LEDA_EXCEPTION(1, "conc_pq_tree::operator=(...): not implemented yet.");
  return *this;
}
...
void conc_pq_tree::conc(conc_pq_tree &pq, int dir) {
  ...
  if (a != pq.a || b != pq.b) 
    LEDA_EXCEPTION(1, "void conc_pq_tree::conc(...): incompatible tree types");
  if (&pq == this) 
    LEDA_EXCEPTION(1, "void conc_pq_tree::conc(...): argument is identical with object"); 

  conc_pq_tree *T1 = (dir == leda::behind ? this : &pq );
  conc_pq_tree *T2 = (dir == leda::behind ? &pq  : this);
...
void conc_pq_tree::split_at_item(conc_pq_node *p, conc_pq_tree &pq1, conc_pq_tree &pq2) {
  ...
  if (root == nil) 
    LEDA_EXCEPTION(1, "void conc_pq_tree::split(...): pq is empty.");
  if (&pq1 == &pq2)
    LEDA_EXCEPTION(1, "void conc_pq_tree::split(...): identical arguments.");
  if (a != pq1.a || a != pq2.a || b != pq1.b || b != pq2.b) 
    LEDA_EXCEPTION(1, "void conc_pq_tree::split(...): incompatible priority queues."); 
...
/*{\Xanpage {concat_pq} {P, I} {Concatenable Priority Queues} {Q}}*/
...
/*{\Xtext 
We implemented a data structure |\Xtype| supporting all needed 
...
of |\Xtype| such that it also enables the maintenance of the surface graph.
...
number of items in the $(a,b)$--tree.
  ...
  /*{\Xdefinition
    An instance |\Xvar| of the parameterized data type |\Xname| is a collection of items
    (type |c_pq_item|). 
    ...
    and information $i$.
    \ignore{|P| is called the priority type and |I| the information type of |\Xvar|.}
    ...
    An item $\Litem{p,i}$ with |p=infinity| is {\it irrelevant} to |\Xvar|. 
    The number of items in |\Xvar| is called the |size| of |\Xvar|. |\Xvar| is {\it empty} 
    when all its items are irrelevant, or when |\Xvar| has size zero. 
    A setable generic pointer |owner| (type |void*|) is associated with |\Xvar|. 
  }*/
...
/* {\Xtypes 5} */ 
  ...
  /* {\Xtypemember   the item type.} */

  typedef P prio_type;  
  /* {\Xtypemember   the priority type.} */  

  typedef I inf_type;
  /* {\Xtypemember   the information type.} */ 
...
/*{\Xcreation Q 6.2}*/
  ...
  /*{\Xcreate   creates an instance |\Xvar| of type |\Xname| based on the linear
  ...
  /* {\Xcreate   creates an instance |\Xvar| of type |\Xname| based on the linear order
    defined by the compare function |cmp| and sets the designated element |infinity| 
    to |infty|. |\Xvar| is initialized with the empty priority queue.
...
/*{\Xoperations 1.8 4.4}*/   
  ...
  /* {\Xop   returns a reference to the value of |infinity|.} */

  c_pq_item init(P p, I i) {
    return conc_pq_tree::init(leda_cast(p), leda_cast(i));
  }
  /*{\Xop   initializes |\Xvar| to the priority queue containing 
  ...
  /*{\Xop   returns the priority of item $it$.
    \precond $it$ is an item in |\Xvar|.}*/
  ...
  /*{\Xop   returns the information of item $it$.
    \precond $it$ is an item in |\Xvar|.}*/

  virtual c_pq_item insert(P p, I i) {
    return conc_pq_tree::insert(leda_cast(p), leda_cast(i));
  }
  /* {\Xop   adds a new item $\Litem{p, i}$ to |\Xvar| and returns it. All items in
    |\Xvar| precede $\Litem{p, i}$.} */


  void concat(concat_pq<P,I> &pq, int dir = leda::behind) {
    conc_pq_tree::conc(pq, dir);
  }
  /*{\Xop   concatenates |\Xvar| with |pq|. The items in |\Xvar| precede (succeed)   
  ...
  /*{\Xopl  splits |\Xvar| at item |it| into |pq1| and |pq2| such that |it| is 
    the last item of |pq1|. 
    In case $|it|=|nil|$, |pq2| becomes |\Xvar| and |pq1| becomes empty.
    The instance |\Xvar| is empty thereafter, unless it is given as one
    of the arguments.}*/

  virtual c_pq_item find_min() const { return conc_pq_tree::find_min(); }
  /*{\Xop   returns an item with minimal priority (|nil| if |\Xvar| is empty).}*/
  ...
  /*{\Xop   makes the item $|it|=|\Xvar.find_min|()$ irrelevant to |\Xvar| by
  ...
  /*{\Xopl   makes the item $it$ irrelevant to |\Xvar|.
    \precond |it| is an item in |\Xvar|.}*/

  virtual bool decrease_p(c_pq_item it, const P& x) { 
    return conc_pq_tree::decrease_key(it, leda_cast(x)); 
  }
  /*{\Xopl   makes $x$ the new priority of item $it$. The function returns |true| iff
    the operation was successful, i.e.~|\Xvar.prio(it)| was larger than $x$.}*/

  virtual bool increase_p(c_pq_item it, const P& x) { 
    return conc_pq_tree::increase_key(it, leda_cast(x)); 
  }
  /*{\Xopl   makes $x$ the new priority of item $it$. The function returns |true| iff
    the operation was successful, i.e.~|\Xvar.prio(it)| was smaller than $x$.}*/
  ...
  { return LEDA_ACCESS(I, conc_pq_tree::inf(it)); }
  /* {\Xarrop   returns a reference to the information of item $it$.
    \precond $it$ is an item in |\Xvar|.} */


  int  size() const { return conc_pq_tree::size(); }
  /*{\Xop   returns the size of |\Xvar|.}*/


  bool empty() const { return conc_pq_tree::empty(); }
  /*{\Xop   returns |true|, if |\Xvar| is empty, and |false| otherwise.}*/


  void reset() { conc_pq_tree::reset(); }
  /*{\Xop   makes |\Xvar| the empty priority queue by setting all priorities to |infinity|.}*/


  void clear() { conc_pq_tree::clear(); }
  /*{\Xop   makes |\Xvar| the empty priority queue by deleting all items. }*/


  void set_owner(GenPtr pt) { conc_pq_tree::set_owner(pt); }
  /*{\Xop   sets |owner| of |\Xvar| to the object pointed to by the 
            generic pointer |pt| (type |void*|).}*/


/*{\Xtext \mansection{Friend Functions}}*/

/*
  friend GenPtr get_owner(c_pq_item it);
*/
  /*{\Xfunc returns the generic pointer |owner| of the instance containing item |it|.}*/


/*{\Xtext \mansection{Iteration}
...
/* {\Xfunc   returns the root of item |it|.} */
...
/* {\Xfunc   returns a pointer to the instance of type |\Xname| that contains 
  item |it| in |pq|.} */
  ...
  forall_nodes(r, G) {
    ...
    if (debug) print_pqs<NT>(delta1, resp_d1, delta2a, resp_d2a, delta2b,
      ...
      if (delta2a == Delta) { 
        ...
        #ifdef _STAT
	  blossoms_in_trees--;   // RESP is non-trivial 
        #endif
...
public:
  ...
  int contained_nodes() const { return size(); }
  ...
  node best_adj(c_pq_item it) const { return inf(it)->min_inf(); }
  ...
  bool improve_connection(c_pq_item it, NT x, node u, node r) {
    ...
    if (!it) return false;
    NT old_min = min_prio();
    if (inf(it)->decrease_p1(u, x, r)) decrease_p(it, x);    
    return old_min != min_prio();    
  }
...
public:
  ...
  ~tree() { 
    // GSchange 
    my_blossoms.clear(); d3b_edges.clear(); 
  }
  ...
  #ifdef _STAT
    nodes_in_trees -= CUR->contained_nodes();
    if (!CUR->trivial()) blossoms_in_trees--;
  #endif
  ...
  #ifdef _STAT
    destroy_t += used_time(destroy_h);
    // if (debug) printf("nodes = %d\t blossoms = %d\n", nodes_in_trees, blossoms_in_trees);
  #endif
  }
...
expand_t = augment_t = scan_t = destroy_t = 0;
  ...
  potential_updates = nodes_in_trees = blossoms_in_trees = 0;
  max_nodes_in_trees = max_blossoms_in_trees = 0;
#endif
  ...
  while (free) {
    ...
    if (cand3a == Delta) {
      ...
      if (RESP->label == even) {
        ...
        if (!RESP->trivial())
          RESP->item_in_pq = 
            delta4.insert(compute_potential(RESP, Delta)/2 + Delta, RESP);
        #ifdef _STAT
          nodes_in_trees += RESP->contained_nodes();
          if (!RESP->trivial()) blossoms_in_trees++;
        #endif
        ...
        #ifdef _STAT
          nodes_in_trees += MATE->contained_nodes();
          if (!MATE->trivial()) blossoms_in_trees++;
        #endif
        #ifdef _STAT
        grow_t += used_time(grow_h);
        // if (debug) printf("nodes = %d\t blossoms = %d\n", nodes_in_trees, blossoms_in_trees);
      ...
      forall(CUR, sub1) {
        ...
        #ifdef _STAT
          if (!CUR->trivial()) blossoms_in_trees -= 1;
        #endif	        
      ...
      #ifdef _STAT
        // add the number of immediate subblossoms (which is too much; see below);
        blossoms_in_trees += RESP->subblossom_p.size();
        blossoms_in_trees -= 1;  // RESP is destroyed
      #endif 
      ...
      while (!RESP->subblossom_p.empty()) {
        ...
        #ifdef _STAT
        nodes_in_trees -= CUR->contained_nodes();
        blossoms_in_trees--;  // due to overcounting
        #endif
  ...
  alg_t = used_time(alg_t);
	...
	return M;   // return empty matching
      ...
      #ifdef _STAT
        if (delta - Delta != 0) { 	
          potential_updates += nodes_in_trees + blossoms_in_trees;
/*
          if (nodes_in_trees + blossoms_in_trees > max_nodes_in_trees + max_blossoms_in_trees) {
            max_nodes_in_trees = nodes_in_trees;
            max_blossoms_in_trees = blossoms_in_trees;
          }
*/
        }
        // if (debug) printf("nodes = %d\t blossoms = %d\n", nodes_in_trees, blossoms_in_trees);
        // if (debug) cout << "potential updates = " << potential_updates << endl;
      #endif
...
#ifdef _INFO
  if (check) cout << "CHECK MAX WEIGHT MATCHING..." << std::flush;
#endif
  check_t = used_time();
...
#ifdef _INFO
  if (check) cout << "CHECK MAX WEIGHT PERFECT MATCHING..." << std::flush;
#endif
  check_t = used_time();
...
#ifdef _INFO
  if (check) cout << "CHECK MIN WEIGHT PERFECT MATCHING..." << std::flush;
#endif
  check_t = used_time();
...
bool CHECK_MAX_WEIGHT_MATCHING_T(const graph &G,
  ...
  return CHECK_MWM_T(G, w, M, pot, BT, b, false, "CHECK_MAX_WEIGHT_MATCHING: ");
}
...
bool CHECK_MAX_WEIGHT_PERFECT_MATCHING_T(const graph &G,
  ...
  return CHECK_MWM_T(G, w, M, pot, BT, b, true, "CHECK_MAX_WEIGHT_PERFECT_MATCHING: ");
}
...
bool CHECK_MIN_WEIGHT_PERFECT_MATCHING_T(const graph &G,
  ...
  return CHECK_MWM_T(G, w_mod, M, pot, BT, b, true, "CHECK_MIN_WEIGHT_PERFECT_MATCHING: ");
}
