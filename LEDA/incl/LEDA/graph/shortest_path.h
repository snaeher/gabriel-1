...
extern __exportF bool K_SHORTEST_PATHS(graph& G, node s, node t, const edge_array<int>& c,
	int k, list<list<edge>* >& sps, int& nops);
/*{\Mfuncl
K\_SHORTEST\_PATHS solves the k shortest simple paths problem in the 
graph $G(V,E)$ with respect to the source node $s$, the target node $t$, and the cost-function given
by the edge\_array |c|. $k$ is an input parameter specifying the number of paths to be computed.

$sps$ reports the $nops$ shortest simple paths computed, each specified as a list of edges from $s$ to $t$.
$nops$ is an output parameter that gives the number of reported paths. It is usually $k$, except in the case that
there are more than $k$ shortest paths of the same length, then all of them are reported or in the case that 
there are less than $k$ paths from $s$ to $t$. In both cases, $nops$ deviates from $k$ and specifies the number of reported paths.
}*/
LEDA_END_NAMESPACE
...
extern __exportF rational MINIMUM_RATIO_CYCLE(graph& G, 
                                              ...
                                              list<edge>& C_start);
/*{\Mfunc Returns a minimum cost to profit ratio cycle |C_start| and the
...
profit of any edge, respectively. The function returns zero if there
is no cycle in $G$.\\
