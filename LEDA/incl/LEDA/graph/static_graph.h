...
the computation
of the source or target node of a given edge but allow walking from
one
      ...
      : public st_edge_struct<directed_graph> 
{
  ...
  template <class edge>
  edge next_in_edge(edge e) const { return (edge) NextInEdge;     }
...
class st_edge_struct<opposite_graph> 
{
  ...
  { return node((size_t)v ^ (size_t)srcXtgt); }
...
{ node v;
  ...
  edge e;
  forall_edges(e,(*this)) e->NextInEdge = e_end;
}
...
bool test_in_edge(edge e) const { return e->NextInEdge != e_end; }
