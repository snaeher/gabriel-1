...
#define LEDA_ROOT_INCL_ID 661124
...
extern __exportF bool FEASIBLE_FLOW(const graph& G, const node_array<int>& supply,
                                              const edge_array<int>& lcap,
                                              const edge_array<int>& ucap,
                                              edge_array<int>& flow);
/*{\Mfuncl
FEASIBLE\_FLOW takes as arguments a directed graph $G$, two edge\_arrays
$lcap$ and $ucap$ giving for each edge a lower and upper capacity bound,
...
and mass balance conditions) exists it computes such a $flow$
...
inline bool MIN_COST_FLOW(const graph& G, const edge_array<int>& lcap,
                                          const edge_array<int>& ucap,
                                          const edge_array<int>& cost,
                                          const node_array<int>& supply,
                                          edge_array<int>& flow)
...
inline bool MIN_COST_FLOW(const graph& G, const edge_array<int>& cap,
                                          const edge_array<int>& cost,
                                          const node_array<int>& supply,
                                          edge_array<int>& flow)
{ 
  ...
  return MCF_COST_SCALING(G,lcap,cap,cost,supply,flow); 
 }
/*{\Mfuncl
...
inline int MIN_COST_MAX_FLOW(const graph& G, node s, node t, 
                                             const edge_array<int>& cap, 
                                             const edge_array<int>& cost, 
                                             edge_array<int>& flow)
